...
class QDA_PLT(Model):
	...
	def build(self,reg_param,store_covariance):
		return QuadraticDiscriminantAnalysis(reg_param=reg_param,store_covariance=store_covariance)
	
	# running the model    
	def run(self,epochs=0):
		# 3 enzynmes * 3 descriptors * 2 scalers * 4 splits = 72
		# 72 * 6 * 2 = 
		grid={
			"reg_param" : [1e-2, 1e-1, 0.25, 0.50, 0.75, 1.0],
		...
		for p in list(ParameterGrid(grid)):
			self.data.params=p
			self.data.sign=f"{p['reg_param']}-{p['store_covariance']}"
			...
			self.model=self.build(reg_param=p["reg_param"],store_covariance=p["store_covariance"])
			self.model.fit(self.data.x_train,self.data.y_train)
			self.model=CalibratedClassifierCV(estimator=self.model,method="sigmoid")
			self.model.fit(self.data.x_train,self.data.y_train)
...
def main():
	...
	model=QDA_PLT(data,predictions)
	model.process_files()
	return
