...
def process(model_selection):
	for i in range(len(dbs)):
		...
		for enzyme in enzymes:
			...
			for descriptor in descriptors:
				...
				for split in splits:
					...
					for phase in phases:
						if phase == "test":
							path=folder+"/data-decomposed/"+model_selection+"-"+enzyme+"-"+descriptor+"-x_"+phase+".csv"
						else:
							path=folder+"/data-decomposed/"+model_selection+"-"+enzyme+"-"+descriptor+"-x_work-"+str(split)+"-"+phase+".csv"
		...
		for model in models:
			print(db,model)
			path=folder+"/predictions/"+model_selection+"/"+model+"-pred.csv"
			...
			for enzyme in enzymes:
				...
				for descriptor in descriptors:
					...
					path=folder+"/data-decomposed/"+model_selection+"-"+enzyme+"-"+descriptor+"-y.csv"
					...
					for scaler in scalers:
						...
						for split in splits:
							...
							for signature in signatures:
								...
								print(db,model,enzyme,descriptor,scaler,model_selection,split,signature,df2.shape)
								...
								for phase in phases:
									...
									precision,recall,thresholds=metrics.precision_recall_curve(df44["pIC50"],df22["y_pred_int"])
			...
			# 	df5.to_csv(folder+"/metrics/metrics-"+model_selection+"-"+model+"-"+phase+".csv",index=False)
			df3.to_csv(folder+"/metrics/metrics-"+model_selection+"-"+model+".csv",index=False)
	return
