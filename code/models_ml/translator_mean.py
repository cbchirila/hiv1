...
# the mean is computed on all splits and on all hyper-parameters in two steps
...
def translate(model_selection,model,input,output):
	...
	for enzyme in enzymes:
		for descriptor in descriptors:
			df2=df1[(df1["enzyme"]==enzyme) & (df1["descriptor"]==descriptor) & (df1["scaler"]=="std")]
			...
			folder2=output+"/"+model_selection+"/"+model+"/"
			create_directory(folder2)
			file2=folder2+"/"+model+"-"+enzyme+"-"+descriptor+".csv"
			df3.to_csv(file2,index=False)
	return

def translate_data_mean():
