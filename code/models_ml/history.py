...
class History():
    ...
    def add(self,h):
        ...
        df_h["signature"]=pd.Series([self.data.signature for i in range(len(df_h))])
    ...
    def save(self):
        ...
        self.df.to_csv(self.data.root+"/history/"+self.data.modelName+"-"+self.data.database+"-"+self.data.descriptor+"-acc-loss-hist"+
    ...
    def plot(self,h):
        ...
        plt.title(self.data.modelName.upper()+" "+self.data.descriptor+" "+self.data.scalerName+" "+str(self.data.split))
        ...
        self.data.modelName+"/"+self.data.descriptor+"-"+self.data.modelName+"-"+self.data.scalerName+"-"+str(self.data.split)+
        "-acc-loss"+
        #"-"+datetime.datetime.now().strftime("%Y.%m.%d-%H.%M.%S")+"-"+socket.gethostname().lower()+
        "")
        plt.cla()
    ...
    def clean(self):
        ...
        d=self.data.descriptor
        p=r+"/history/"+m+"-"+db+"-"+d+"-acc-loss-hist-ch.csv"
        if os.path.exists(p):
            os.remove(p)
        return
